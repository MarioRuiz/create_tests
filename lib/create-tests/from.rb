...
class CreateTests
  ...
  # @param spec_dir [String]. (default './spec'). Output directory for generated test files.
  ...
  ##############################################################################
  def self.from(requests_file, type: :request_hash, test: :rspec, mode: :append, dry_run: false, return_data: false, spec_dir: './spec', settings_dir: './settings', cleanup: :after_all)
    ...
    if mode == :overwrite
      ...
      if Dir["#{spec_dir}/*/**_spec.rb"].size > 0
    ...
    Dir.mkdir spec_dir unless no_write or test != :rspec or Dir.exist?(spec_dir)
    ...
    modules.each do |mod_txt|
      ...
      mod_methods_txt.each do |method_txt|
        ...
        if File.exist?(test_file) and mode==:dont_overwrite
          ...
          modified, txt = create_test(mod_txt, method_txt, mod_obj.method(method_txt), test_txt, settings_relative_path: settings_from_test, cleanup: cleanup)
          if no_write
    ...
    if add_helper
      ...
      if @setup_cleanup_resources.size > 0
        write_chain_file = lambda do |path, content, label|
          if File.exist?(path) && mode != :overwrite
            message = "** The file #{path} already exists so it will be left unchanged.\n"
            message += "   Remove the file to regenerate it or set mode: :overwrite"
            @logger.warn message
            warn message
            return
          end
